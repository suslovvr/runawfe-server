...
public class ExecutionContext {
    ...
    private CurrentVariableLog setSimpleVariableValue(CurrentToken token, VariableDefinition variableDefinition, Object value) {
        ...
        if (variable == null) {
            ...
            if (!(variable.getStorableValue() instanceof byte[])) {
                ...
                updateMessageSelectorIfExists(variable, token.getProcess(), oldValue);
            }
            VariableDefinition syncVariableDefinition = subprocessSyncCache.getParentProcessSyncVariableDefinition(
                    token.getProcess(), variableDefinition
            );
    ...
    private void updateMessageSelectorIfExists(CurrentVariable<?> variable, CurrentProcess process, Object oldValue) {
        List<CurrentToken> tokenList = currentTokenDao.findByProcessAndMessageSelectorLikeAndExecutionStatusIsNotEnded(process,
                Utils.MESSAGE_SELECTOR_VALUE_DELIMITER + TypeConversionUtil.convertTo(String.class, oldValue));
        for (CurrentToken token : tokenList) {
